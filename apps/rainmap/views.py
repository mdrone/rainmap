...
from apps.rainmap import tasks

from apps.rainmap.models import UserProfile, Scan, ScanResult
from apps.rainmap.forms import UserForm, ScanForm
...
def index(request):
    if request.user.is_authenticated():
        ...
        return render_to_response('rainmap/dashboard.html',
            {'result_list': result_list},
            context_instance=RequestContext(request))
    else:
        return render_to_response('rainmap/guestview.html',
            context_instance=RequestContext(request))


@login_required(redirect_field_name='next')
def profile(request, template='rainmap/profile.html'):
...
@login_required(redirect_field_name='next')
def scan_list(request, template='rainmap/scan_list.html'):
...
@login_required(redirect_field_name='next')
def scan_view(request, scan_id, template='rainmap/scan_view.html'):
    ...
    if requested_scan:
        ...
        return redirect('rainmap_scan_list')


@login_required(redirect_field_name='next')
def scan_edit(request, scan_id, template='rainmap/scan_edit.html'):
    requested_scan = _get_scan(request, scan_id)
    if requested_scan:
...
@login_required(redirect_field_name='next')
def scan_copy(request, scan_id, template='rainmap/scan_list.html'):
    requested_scan = _get_scan(request, scan_id)
    if requested_scan:
...
@login_required(redirect_field_name='next')
def scan_delete(request, scan_id, template='rainmap/scan_list.html'):
    requested_scan = _get_scan(request, scan_id)
    if requested_scan:
...
@login_required(redirect_field_name='next')
def scan_new(request, template='rainmap/scan_new.html'):
    form = ScanForm(request.user, request.POST or None)
    if form.is_valid():
...
@login_required(redirect_field_name='next')
def scan_run(request, scan_id, template='rainmap/scan_list.html'):
    next = request.META.get('HTTP_REFERER', None) or 'rainmap_scan_list'
    requested_scan = _get_scan(request, scan_id)
    if requested_scan:
        result = ScanResult(for_scan=requested_scan)
        result.started_on = datetime.datetime.now()
        result.save()
        ...
        rel_dir = os.path.join(str(requested_scan.owner.id), str(requested_scan.id))
        ...
        tasks.run_scan.delay(name_base, workdir, scan_id, request.user.id, result.id, cmd)
...
@login_required(redirect_field_name='next')
def result_view(request, result_id, template='rainmap/result_view.html'):
...
@login_required(redirect_field_name='next')
def result_delete(request, result_id, template='rainmap/result_delete.html'):
    next = request.META.get('HTTP_REFERER', None) or reverse('index')
    if next.find(reverse('rainmap_result_view', args=[result_id])) > 0:
        next = reverse('index')
    try:
